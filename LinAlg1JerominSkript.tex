...
\paragraph{Bemerkung und Definition} In $\mathbb{Z}_2: 1 + 1 = 0$. Allgemeiner definiert man die Charakteristik eines Körpers $(K,+,\cdot)$ (mit neutralen Elementen 0 und 1 von + und $\cdot$) durch
...
z.B. $Char(\mathbb{Z}_2): = \mathbb{Z}:$, da
\begin{gather*}
...
\clip(-0.1,-0.1) rectangle (9,4);

\draw[->,color=black] (-0.3,0) -- (9,0);
...
\draw [->] (3.,1.5) -- (4.,2.5);
\draw [->] (1.,2.) -- (4.,2.5);
\draw (0.8,1.9) node[anchor=north west] {$x$};
\draw (1.5,2.5) node[anchor=north west] {$w$};
\draw (2,1.7) node[anchor=north west] {$v$};
\draw (3.4,1.9) node[anchor=north west] {$w$};
\draw (2.6,2.8) node[anchor=north west] {$v$};
\draw (2.2,2.6) node[anchor=north west] {$v+w$};
\draw (2.3,2.19) node[anchor=north west] {$w+v$};
\draw (2,3.4) node[anchor=north west] {$y' = \tau_w(x)$};
\draw (2.8,1.4) node[anchor=north west] {$y = \tau_v(x)$};
\draw (4.05,2.45) node[anchor=north west] {$z = \tau_w(y)=(\tau_w\circ\tau_v)(x)=\tau_{w+v}(x)$};
\draw (4,2.9) node[anchor=north west] {$z' = \tau_v(y')=(\tau_v\circ\tau_w)(x) = \tau_{v+w}(x)$};
\draw (4,0.5) node[] {Translationen \glqq der\grqq{} Ebene bilden eine abelsche Gruppe};
\draw [fill=blue] (1.,2.) circle (1pt);
\draw [fill=blue] (2.,3.) circle (1pt);
\draw [fill=blue] (3.,1.5) circle (1pt);
\draw [fill=blue] (4.,2.5) circle (1pt);
\end{tikzpicture}
...
\draw [->] (1,3.2) -- (2,2.2);
\draw [->] (2,2.2) -- (3,1.2);
\draw (0.8,3.125) node[anchor=north] {$x$};
\draw (3.2,1) node[anchor=west] {$y = \tau_v(x) = (\tau_{\frac{v}{2}} \circ \tau_{\frac{v}{2}})(x) = \tau_{\frac{v}{2} + \frac{v}{2}}(x)$};
\draw (1.7,2) node[anchor=north] {$v$};
\draw (1.65,2.6) node[anchor=south] {$\frac{v}{2}$};
\draw (2.5,1.75) node[anchor=south] {$\frac{v}{2}$};
\draw (5,3) node[] {Translationen kann man \glqq strecken\grqq{},};
\draw (5,2.7) node[] {sodass gewisse Rechengesetze gelten.};
\end{tikzpicture}
