...
\begin{Definition}[Geometrie]\index{Geometrie}
...
\begin{Definition}[Affiner Raum]\index{Affiner Raum}
...
\subsection{Beispiel \& Definition (affiner Standardraum)}
	\begin{Definition}[Affiner Standardraum]\index{Affiner Standardraum}
...
\subsection{Beispiel \& Definition (Ursprung)}
	\begin{Definition}[Ursprung]\index{Ursprung}
...
\paragraph{Beispiel}
	...
	keinen affinen Raum, da diese Operation nicht einfach transitiv ist: zu je zwei Punkten gibt es unendlich viele "`Verbindungsvektoren"'.
 	...
 	\begin{tikzpicture}[scale=1.8,tdplot_main_coords]
 		...
 		{$(x,y)\sim (\tilde{x},\tilde{y})$ \\ $:\Leftrightarrow
 			\begin{cases} \exists k \in \mathbb{Z}: &  \tilde{x} =x + 2k \pi, \\  & \tilde{y} = y \end{cases}
 		$};
...
\subsection{Beispiel \& Definition (affiner Unterraum)}
	...
	\begin{Definition}[Affiner Unterraum]\index{Affiner Unterraum}
...
\subsection{Definition (allgemeine Lage)}
	\begin{Definition}[Allgemeine Lage]\index{Allgemeine Lage}
...
\subsection{Lemma (Eindeutigkeit der Punktdarstellung)}
	\begin{Lemma}[Eindeutigkeit der Punktdarstellung]\index{Eindeutigkeit der Punktdarstellung}
