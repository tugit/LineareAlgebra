	...
	Für (i) wird nur die Existenz der Adjungierten benutzt.
		\begin{enumerate}[(i)]
			...
			Damit ist es sinnvoll
				...
				\[ f^*\big|_U\in \End(U), f^*\big|_{U^\perp}\in \End(U^\perp) \]
			zu betrachten. Nun gilt\footnote{Es ist nicht klar ob $f|_U^*$ existiert, daher ist die folgende Rechnung von hinten zu lesen}:
...
\paragraph{Bemerkung \& Beispiel}
	 Eine Orthogonalprojektion ist selbstadjungiert, $ p\in \End(V) $ mit $ p^2=p, p^*=p $ und damit normal. Ist $ p\neq \id_V, 0 $, so ist
		 \[ V=U\oplus_\perp U^\perp \quad\text{mit}\quad
			 \begin{cases}
			 U:= p(V),\\ U^\perp = \ker p.
			 \end{cases} \]
	Wir definieren:
...
\end{Korollar}
\paragraph{Beweis}
	Sei $ X\in \C^{n\times n} $, betrachte $ \C^n $ als unitären VR mit Standardbasis $ E $ als ONB.\footnote{Die folgende Gleichung definiert das Skalarprodukt.}
		\[ \Gamma_E(\Skl{.}{.}) = E_n, \]
	und den assoziierten Endomorphismus $ f_X\in \End(\C^n) $. Orthonormale Basiswechsel $ B=EP $ sind dann durch unitäre Matrizen $ P\in U(n) $ gegeben:\footnote{$B$ soll ONB sein}
...
\begin{Satz}[Spektralzerlegung (Euklidischer Fall)]
	Seien $ (V,\Skl{.}{.}) $ Euklidischer VR mit $ \dim V <\infty $ und $ f\in \End(V) $ normal; dann zerfällt $ V $ als orthogonale direkte Summe $ f $- und $ f^* $-invarianter UVR $ V_i $
		\[ V = \bigoplus_{i=1}^mV_i \quad\text{mit}\ V_i\perp V_j \text{ für } i\neq j \]
	und
		\[ \begin{cases}
...
\subsection{Buchhaltung}
	...
	gibt es also eine ONB $ E $ von $ (V,\Skl{.}{.}) $, sodass 
		\[ \xi_E^E(f) = \begin{pmatrix}
			x_1    & 0      & \cdots & \cdots  & \cdots & 0      \\
			0      & \ddots & \ddots &         &        & \vdots \\
			\vdots & \ddots & x_k    & \ddots  &        & \vdots \\
			\vdots &        & \ddots & X_{n+1} & \ddots & \vdots \\
			\vdots &        &        & \ddots  & \ddots & 0      \\
			0      & \cdots & \cdots & \cdots  & 0      & X_m
		\end{pmatrix}
...
\paragraph{Beweis}
	...
	\begin{itemize}
		...
		dann ist
			...
			&\overset{\footnotemark}{=} \Skl{\left((f^*)^{-1}\circ f^{-1}\circ f\right)(v)}{f(v)} \overset{\footnotemark}{=} \Skl{v}{\left(f^{-1}\circ f\right)(v)} = \Skl{v}{v},
			\end{align*}
			\footnotetext{$(h^{-1})^*=(h^*)^{-1}=h^{-1}$, (selbstadjungiert)}
			\footnotetext{$(f^*)^{-1}=(f^{-1})^*$}
		also ist $ k\in O(V) $.
	\end{itemize}
\paragraph{Bemerkung}
