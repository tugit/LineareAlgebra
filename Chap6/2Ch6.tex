...
\paragraph{Beweis}
	...
	\begin{itemize}
		...
		dann ist
			...
			&\overset{\footnotemark}{=} \Skl{\left((f^*)^{-1}\circ f^{-1}\circ f\right)(v)}{f(v)} = \Skl{v}{\left(f^{-1}\circ f\right)(v)} = \Skl{v}{v},
			\end{align*}
			\footnotetext{$(h^{-1})^*=(h^*)^{-1}=h^{-1}$, (selbstadjungiert), sowie $(f^*)^{-1}=(f^{-1})^*$}
		also ist $ k\in O(V) $.
	\end{itemize}
\paragraph{Bemerkung}
