...
\paragraph{Beweis: }
	...
	und $p^\prime p = 0 $. Weiters liefert
		\[\forall v\in V: v=id_v(v) = p(v) + p^\prime(v) \Rightarrow V = p(V)+p^\prime(V),\]

	und ist $w = p(V)= p^\prime(v^\prime)$ für geeignete $v,v^\prime\in V$ (d.h., $w  = p(V)\cap p^\prime(V))$ so gilt
		\[w = p(v) = p^2(v) = p(p(v)) = p(w) = p(p^\prime(v^\prime)) = pp^\prime(v^\prime) = 0 (pp^\prime = 0),\]
...
\paragraph{Bemerkung: }
		Im Beweis haben wir etwas mehr bewiesen als behauptet - nämlich:
			\[ ker p = p^\prime(V)\text{ und }\ker p^\prime = p(V) \]
