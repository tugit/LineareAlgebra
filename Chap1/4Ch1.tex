...
\subsection{Beispiel und Definition (Dualraum)}
	\begin{Definition}[Dualraum]
		Der \emph{Dualraum} $V^\ast := \hom(V,K)$ eines $K$-VRs $V$ ist ein $ K $-VR $(\subset K^V)$. Ist $\dim V=:n<\infty$ so ist $\dim V^\ast=n$.
                ...
                Nämlich: Ist $B=(b_i, ... ,b_n)$ eine Basis von $ V (\dim V < \infty)$, so definieren wir für $ i = \{1, ... ,n\} $ die Linearform (nach Fortsetzungssatz):
                        \[b_i^\ast\in V^*:V\to K, \forall j\in \{1,...,n\}:b_i^*(b_j)=\delta_{ij}\]
                eine Basis $B^*=(b^*_i)_{i\ in I}$ von $V^*$: die zu $B$ \emph{duale Basis} $ B^* $ von $V^\ast$.
	\end{Definition}
...
\paragraph{Bemerkung}
	Ist $\dim V = \infty$ und $B=(b_i)_{i\in I}$ eine Basis von $V$, so liefert $B^\ast=(b_i^\ast)_{i\in I}$ mit 
        ...
        eine lineare unabhängige Familie, die jedoch kein Erzeugendensystem von $V^\ast$ ist:
        
        $f\in\hom(V,K)=V^\ast$ mit $\forall j\in I:f(b_j)=1$ lässt sich nicht in $B^\ast$ linear kombinieren. Wäre $f=\sum_{i\in I}b_i^\ast x_i$, so gälte
            \[\forall j\in I: x_j =\sum_{i\in I}b_i^\ast(b_j)x_j= \sum_{i\in I} \delta_{ij}x_j = f(b_j) = 1\]
...
\paragraph{Beweis} 
	...
	\begin{align*}
            ...
            &\Rightarrow \rg f = \dim V-0 = \dim V\\
            &\Rightarrow f(V) = W \Leftrightarrow f \text{ surjektiv}
        \end{align*}
        \begin{align*}
...
\paragraph{Bemerkung}
	...
	\begin{align*}
		...
		&\Rightarrow 0 = f(v) = f(\sum_{b\in B}bx_b) = \sum_{b\in B}f(b)x_b\\
		&\Rightarrow \forall b \in B: x_b = 0 \Rightarrow v = 0, \text{ d.h., } \ker f=\{{0}\} \Rightarrow f \text{ inj.}.
	\end{align*}
...
\paragraph{Bemerkung: }
	...
	\begin{gather*}
		(f\circ f^{-1})(\sum_{i\in I}v_ix_i) = f(\sum_{i\in I}f^{-1}(v_i)x_i) = \sum_{i\in I}\underbrace{(f\circ f^{-1})}_{= id}(v_i)x_i\\
