...
\subsection{Bemerkung und Definition (Familienschreibweise)}
	...
	für die Auswertung einer Abbildung  $v: I \to K$ um einen Punkt $i\in I$ haben wir die Indexschreibweise verwendet.	
		\[ I\ni i \mapsto v_i \in K \]
	Wir haben damit eine Abbildung $v: I \to K$ als Familie von $ (v_i)_{i\in I} $ über der Indexmenge $ I $ aufgefasst -- der Begriff Familie ist ein \glqq alternativer\grqq{} Begriff für Abbildungen.
	\end{Definition}
...
\paragraph{Beispiel}
	...
	denn
		\[ \forall j\in I: \left(\sum^n_{i=1} e_iv(i)\right)(j) = \sum^n_{i=1}e_i(j)v(i) = \sum_{i=1}^n \delta_{ij} v(j) = v(j) \]
...
\subsection{Definition (Linearkombination)}
	...
	\begin{gather*}
    	...
    	\text{Dann heißt die endliche Summe }\sum_{i\in I} v_ix_i:= \sum^n_{j=1}v_{i_j}x_{i_j}\text{ eine Linearkombination.}
	\end{gather*}
	\end{Definition}
...
\paragraph{Beweis}
	...
	$\supset$: z.z.: jede Linearkombination liegt in der Linearen Hülle. Sei also $(x_i)_{i\in I}$ eine geeignete Familie in $ K $, dann gilt:
	...
	$\subset$: Z.z.: Lineare Hülle der Familie ist Teilmenge von U. Setze die Menge der Linearkombinationen einer Familie U $:= \{{\sum_{i\in I} v_ix_i| x: I\to K \text{ mit } \#\{{i\in I| x_i \neq 0\}} < \infty\}}$, offenbar gilt:
...
\subparagraph{Zur Skalarmultiplikation}
	...
	\begin{gather*}
		...
		\Rightarrow \sum_{j=1}^{n} v_{i_j}(x_{i_j}x) = \sum_{i\in I} v_i(x_ix) \in U_i,
	\end{gather*}

	da $\sum_{i\in I} v_i(x_ix)$ Linearkombination (mit der Familie $(x_ix)_{i\in I}$ in K) ist.

\subparagraph{Zur Addition}
